...
async function createWidget(items) {
		...
		if(!data.batteryStatus || typeof(data.batteryStatus) == "undefined"){
			if (config.runsInWidget) { // only in widget
				throw new Error('Quota Limit! – API call currently not possible. Try again later.')
			} else {
				console.log("Quota Limit! – API call currently not possible. Try again later.");
			}
		}
		...
		if(typeof(data.batteryStatus) != 'undefined'){
			...
			const RangeStatusLabel = RangeStack.addText("Räckvidd")
		...
		if(ZOE_Phase == 1 && typeof(data.batteryStatus) != 'undefined'){
			if(typeof(data.batteryStatus.attributes.batteryTemperature) != 'undefined'){
				...
				const TempStatusLabel = TempStack.addText("BAtteri Temp.")
		...
		if(typeof(data.cockpitStatus) != 'undefined'){
			...
			const MileageStatusLabel = MileageStack.addText("Kilometers")
		...
		if(typeof(data.locationStatus) != 'undefined'){
			...
			const LocationVal = LocationStack.addText("➤ Öppna karta")
		...
		//if(typeof(data.hvacStatus) != 'undefined'){ // we have to uncomment this later!
			...
			const AcLabel = AcStack.addText("Luftkonditionering")
			...
			if(args.queryParameters.action == 'start_ac'){
					AcVal = AcStack.addText("➤ Stoppa luftkon")
					ac_url = `scriptable:///run?scriptName=${scriptName}&action=stop_ac`; 
			} else {
				  AcVal = AcStack.addText("➤ Start Luftkon")
...
async function getData() {
		   ...
		   typeof(gigyaCookieValue) == "undefined" || typeof(gigyaPersonID) == "undefined")
		{
			...
			if(apiResult.statusCode == "403"){
				let loginMessage = "Login not possible. Check login information (username, password)"
		...
		if(account_id == "" || typeof(account_id) == "undefined"){
			...
			if(apiResult.type == "FUNCTIONAL"){
				let quotaMessage = apiResult.messages[0].message + " – Login currently not possible try again later."
...
async function getStatus(endpoint, version=1, kamareonURL, account_id, VIN, gigyaJWTToken, kamareonAPI ){
	...
	url = kamareonURL + '/commerce/v1/accounts/' + account_id + '/kamereon/kca/car-adapter/v' + version + '/cars/' + VIN + '/' + endpoint + '?country=SE'
	req = new Request(url)
	req.method = "GET"
...
async function postStatus(endpoint, jsondata, version, kamareonURL, account_id, VIN, gigyaJWTToken, kamareonAPI ){
	url = kamareonURL + '/commerce/v1/accounts/' + account_id + '/kamereon/kca/car-adapter/v' + version + '/cars/' + VIN + '/actions/' + endpoint + '?country=SE'
	...
	if (request.response.statusCode == 200) {
		pushBody = "The request was sent successfully"
		sound = "piano_success"
	} else {
		pushBody = "Something went wrong sending the request. Code:" + request.response.statusCode
		sound = "piano_error"
	}
	...
	if(endpoint == "hvac-start"){
		pushMessage.title = "Sent command to HVAC"
	}
